...
export function buildPremiumEmailPreviewHtml(options: {
  ...
  const subtitleHtml = subtitle
    ? `<p style="margin:12px 0 0;font-size:14px;color:rgba(255,255,255,0.92);font-weight:400;text-shadow:0 1px 2px rgba(0,0,0,0.25);">${subtitle}</p>`
    : "";
  ...
  return `<div style="font-family:'Segoe UI',system-ui,-apple-system,sans-serif;max-width:560px;margin:0 auto;background:#eef0f6;border-radius:20px;overflow:hidden;border:1px solid #e2e8f0;box-shadow:0 16px 48px rgba(15,23,42,0.12);">
    ...
    <div style="background:#ffffff;padding:32px 28px;font-size:15px;line-height:1.75;color:#1e293b;">
      ${bodyText}
      ${verifiedBadge}
      ${imgs}
    </div>
    <div style="padding:20px 28px;text-align:center;border-top:1px solid #e2e8f0;background:#f8fafc;">
      <p style="margin:0;font-size:12px;color:#64748b;letter-spacing:0.04em;">${footerNote || "Sent via VerifyID"}</p>
    </div>
  </div>`;
}
